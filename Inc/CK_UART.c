...
void CK_UART_Init1(int baudRate){
	...
	CK_GPIOx_Init(GPIOA,9,CK_GPIO_AF,CK_GPIO_AF7,CK_GPIO_PUSHPULL,CK_GPIO_HIGH,CK_GPIO_NOPUPD);
	CK_GPIOx_Init(GPIOA,10,CK_GPIO_AF,CK_GPIO_AF7,CK_GPIO_PUSHPULL,CK_GPIO_HIGH,CK_GPIO_NOPUPD);

	USART1->BRR = baudNum<<4;//First 4 bit is fraction and not affecting
	USART1->CR1 |= CK_USART_CR1_UE | CK_USART_CR1_OVER8;


	//Later setup USART3 for gps here
	/*RCC->APB1ENR |= CK_RCC_APB1ENR_USART3;
	//Set GPIO pins PC10=TX,PC11=RX
	CK_GPIOx_ClockEnable(GPIOC);
	CK_GPIOx_Init(GPIOC,10,CK_GPIO_AF,CK_GPIO_AF7,CK_GPIO_PUSHPULL,CK_GPIO_HIGH,CK_GPIO_NOPUPD);//TX
	CK_GPIOx_Init(GPIOC,11,CK_GPIO_AF,CK_GPIO_AF7,CK_GPIO_PUSHPULL,CK_GPIO_HIGH,CK_GPIO_NOPUPD);//RX
	USART3->BRR = baudNum_sbus<<4;//First 4 bit is fraction and not affecting
	USART3->CR1 |= CK_USART_CR1_UE | CK_USART_CR1_OVER8;
	USART3->CR1 |= CK_USART_CR1_RXNEIE | CK_USART_CR1_RE;
...
void CK_IntPrintln(int num){
	...
	CK_TXBuffer_int[idx++] = 10;//ASCII new line
